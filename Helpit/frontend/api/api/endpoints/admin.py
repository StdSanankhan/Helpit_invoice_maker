...
from ...models.models import User, Business, Invoice, Subscription, Payment
...
async def approve_payment(payment_id: str, db: AsyncSession = Depends(get_db), admin: User = Depends(get_current_admin_user)):
    ...
    if payment.status != "Pending": raise HTTPException(status_code=400, detail=f"Payment is already {payment.status}")
